...
export function SambaGlobalParser(): Transformer<SambaGlobalConfig, KeyValueData> {
  return {
    apply: (unparsed) => {
      const config = SambaGlobalConfig.defaults();
...
export function SambaShareParser(name: string): Transformer<SambaShareConfig, KeyValueData> {
  return {
    apply: (unparsed: KeyValueData) => {
      const config = SambaShareConfig.defaults(name);
...
export function SambaConfParser(): SyntaxParser<SambaConfig> {
          ...
          .andThen((global) =>
            Result.combine(
              Object.entries(sharesIni).map(([name, shareIni]) =>
                SambaShareParser(name).apply(shareIni)
              )
