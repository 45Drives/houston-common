...
import { QuestionMarkCircleIcon, SunIcon, MoonIcon } from "@heroicons/vue/20/solid";
  ...
  >
    <HoustonHeader :moduleName="moduleName">
      ...
      <template v-slot:header-right>
        <button v-if="showDarkModeButton" @click="darkModeState = !darkModeState">
          <SunIcon v-if="darkModeState" class="size-icon-lg icon-default" />
          <MoonIcon v-else class="size-icon-lg icon-default" />
        </button>
        <component v-if="props.notificationComponent" :is="props.notificationComponent" />
      </template>
    </HoustonHeader>
    <div :class="['grow basis-0 flex flex-col items-stretch', noScroll ? '' : 'overflow-y-auto']">
      <div
        :class="['bg-well grow', noScroll ? '' : 'overflow-y-auto']"
