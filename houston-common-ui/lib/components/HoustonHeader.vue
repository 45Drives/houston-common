...
function onClickHome(e: MouseEvent) {
...
function vape() {
  function makeWide(string: string) {
    ...
    if (bytesIn.indexOf(0xef) !== -1)
      // already wide
      return string;
    bytesIn.forEach((byte) => {
      if (/^[a-z]$/.test(String.fromCharCode(byte))) bytesOut.push(0xef, 0xbd, byte + 0x20);
      else if (/^[A-Z0-9]$/.test(String.fromCharCode(byte))) bytesOut.push(0xef, 0xbc, byte + 0x60);
      else if (String.fromCharCode(byte) === " ") bytesOut.push(0xe2, 0x80, 0x83);
      else bytesOut.push(byte);
    });
...
<template>
  ...
  >
    ...
    <div class="flex basis-32 justify-end items-center grow shrink-0 gap-buttons">
      <LoadingSpinner v-if="globalProcessingState" class="self-center grow-0" />
      <div class="grow"></div>
