...
<template>
  <ol role="list" class="divide-y divide-default rounded-md border border-default md:flex md:divide-y-0">
    <li v-for="(label, stepIdx) in labels" :key="label" class="relative md:flex md:flex-1">
      ...
      <a v-else-if="stepIdx === index" class="flex items-center px-6 py-4 text-sm font-medium" aria-current="step">
        <span
          class="flex size-10 shrink-0 items-center justify-center rounded-full border-2 border-gray-700 dark:border-default">
      ...
      <a v-else @click.prevent="" class="group flex items-center">
        <span class="flex items-center px-6 py-4 text-sm font-medium">
          <span
            class="flex size-10 shrink-0 items-center justify-center rounded-full border-2 border-gray-700 dark:border-default group-hover:border-default">
      ...
      <template v-if="stepIdx !== labels.length - 1">
        ...
        <div class="absolute top-0 right-0 hidden h-full w-5 md:block" aria-hidden="true">
          <svg class="size-full text-default" viewBox="0 0 22 80" fill="none" preserveAspectRatio="none">
